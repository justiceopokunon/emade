...
import type { DiyProject } from "@/lib/diyData";

type DiyClientProps = {
  initialProjects: DiyProject[];
};

export function DiyClient({ initialProjects }: DiyClientProps) {
  const [projects, setProjects] = useState(initialProjects);

  useEffect(() => {
    let active = true;
    fetch("/api/diy", { cache: "no-store" })
      .then((res) => (res.ok ? res.json() : initialProjects))
      .then((data) => {
    ...
    <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-10 px-4 pb-20 pt-12 sm:px-8 sm:pt-16 lg:px-16">
      <AnimatedSection animation="slide-up">
        <header className="flex flex-wrap items-center justify-between gap-4">
          ...
          <Link className="btn-ghost hover-lift" href="/">
            Back home
          </Link>
        </header>
      </AnimatedSection>
      ...
      <AnimatedSection animation="scale" delay={200}>
        <ScrollLoad minHeight={240}>
          <div className="relative h-52 w-full overflow-hidden rounded-3xl border border-white/10 sm:h-64 hover-glow">
            <Image
          ...
          {projects.map((project, index) => (
            ...
            >
              <InteractiveCard className="h-full">
                <div className="glass relative h-full overflow-hidden rounded-3xl border border-white/10 p-6 card-interactive">
                  ...
                  <div className="relative space-y-3">
                    ...
                    <ul className="space-y-2 text-sm text-slate-200">
                      {(Array.isArray(project.steps) ? project.steps : []).map((step, idx) => (
                        <li 
                          key={step} 
                          className="flex gap-2 transition-all duration-300 hover:translate-x-1"
                        >
                          <span className="mt-1 h-1.5 w-1.5 rounded-full bg-lime-400 animate-pulse-glow" />
                          <span>{step}</span>
                        </li>
                      ))}
                    </ul>
                    {project.impact && <p className="text-sm text-slate-400">Impact: {project.impact}</p>}
                    {project.pdfUrl ? (
